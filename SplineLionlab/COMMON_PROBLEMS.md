...
**What Happens** The watermark of SPline is always visible on the website you build; 

**fix** 
"Create a component to load this Spline URL, but do not use @splinetool/react-spline. Instead, inject the web component script from <spline-viewer> and bypass the Shadow DOM using a MutationObserver and querySelector('#logo') to force the watermark removal in React, while also hiding it in global CSS via 'spline-viewer::part(logo)'."
